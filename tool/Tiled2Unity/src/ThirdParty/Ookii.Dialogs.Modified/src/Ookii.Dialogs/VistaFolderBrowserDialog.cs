...
namespace Ookii.Dialogs
{
    ...
    public sealed class VistaFolderBrowserDialog : CommonDialog
    {
        ...
        // [seanba] Added dialog data member. Take care we don't hold a reference to this once it's been released.
        private Ookii.Dialogs.Interop.IFileDialog _dialog;

        // [seanba] Added delegate. Note that path may be empty.
        /// <summary>
        /// Delegate for a folder being selected.
        /// </summary>
        public delegate void FolderSelectedHandler(VistaFolderBrowserDialog dlg, string path);

        // [seanba] Added event.
        /// <summary>
        /// Event for a folder being selected.
        /// </summary>
        ...
        /// <summary>
        /// Custom method for forcing the title
        ...
        // [seanba] - Added this for event callbacks
        internal void DoFolderSelected(string path)
        {
            if (OnFolderSelected != null)
            {
                OnFolderSelected(this, path);
            }
        }

        /// <summary>
        /// Custom method for enabling/disabling the "Ok" button
        /// (This is a hack of Ookii's good work)
        /// </summary>
        public void EnableOk(bool enable)
        {
            // This really sucks, btw. What a complete hack.
            IntPtr ptrUnknown = Marshal.GetIUnknownForObject(_dialog);
            IntPtr ptrWindow;
            Guid guidWindow = typeof(Ookii.Dialogs.Interop.IOleWindow).GUID;
            Marshal.QueryInterface(ptrUnknown, ref guidWindow, out ptrWindow);

            Ookii.Dialogs.Interop.IOleWindow window = (Ookii.Dialogs.Interop.IOleWindow)Marshal.GetObjectForIUnknown(ptrWindow);
            IntPtr hwndDialog;
            window.GetWindow(out hwndDialog);
            IntPtr button = GetDlgItem(hwndDialog, 1);
            EnableWindow(button, enable);
        }

        // [seanba] - added pinvoke stuff
        ...
        protected override bool RunDialog(IntPtr hwndOwner)
        {
            ...
            {
                // [seanba] - cached dialog reference must be nulled if we are to release the com object
                _dialog = null;
                if( dialog != null )
